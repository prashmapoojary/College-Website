...
export default function AdminDashboard() {
  ...
  const [authToken, setAuthToken] = useState(() => (typeof window !== 'undefined' ? localStorage.getItem('admin_token') : null));
  ...
  const headersWithAuth = (token) => (token ? { Authorization: `Bearer ${token}` } : {});

  const apiGet = useCallback(async (path, token = authToken) => {
    const res = await fetch(`${base}${path}`, { headers: headersWithAuth(token) });
    if (!res.ok) throw new Error(`GET ${path} failed`);
    return res.json();
  }, [base, authToken]);

  const apiDelete = async (path, token = authToken) => {
    const res = await fetch(`${base}${path}`, { method: 'DELETE', headers: headersWithAuth(token) });
    if (!res.ok) throw new Error(`DELETE ${path} failed`);
    return res.json();
  };

  const load = useCallback(async (token = authToken) => {
    try {
      const [apps, grs, sugs, st, al, sf, emp, pr] = await Promise.all([
        apiGet('/applications', token),
        apiGet('/grievances', token),
        apiGet('/suggestions', token),
        apiGet('/student', token),
        apiGet('/alumni', token),
        apiGet('/staff', token),
        apiGet('/employers', token),
        apiGet('/parents', token),
      ]);
  ...
  const removeOne = async (id) => {
    ...
    try {
      await apiDelete(`/${route}/${id}`, authToken);
  ...
  const clearAll = async () => {
    ...
    try {
      await apiDelete(`/${view}`, authToken);
      await load(authToken);
    } catch (e) {
