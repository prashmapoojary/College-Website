...
{ year: 1951, name: "Nirmala Pai T", class: "Inter.", rank: "Gold Medal" },
  ...
  return (
    <div className="college-page-container px-4 py-10 font-sans">
      <div className="max-w-5xl mx-auto college-glass-card p-6 md:p-8">
        {/* Header */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8 pb-4 border-b border-amber-500/30">
          <h1 className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-slate-950 via-slate-800 to-slate-950 text-amber-400 py-3 px-6 rounded-xl shadow-xl border-y border-amber-500/40 tracking-wider uppercase">
            🏆 Rank Holders
          </h1>
          <a
            href="/"
            className="px-5 py-2.5 rounded-full bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-bold shadow-md transition text-sm"
          >
            Home
          </a>
        </div>
          ...
          .map((year) => (
            ...
            >
              ...
              <button
                ...
                className={`w-full flex items-center justify-between px-5 py-3.5 font-bold text-left transition rounded-xl ${
                  openYear === year
                    ? "bg-gradient-to-r from-slate-950 via-slate-900 to-slate-800 text-amber-400 border border-amber-500/40"
                    : "bg-white/90 text-slate-800 border border-slate-200 hover:border-amber-500"
                }`}
              >
                <span className="text-base sm:text-lg">Year {year}</span>
                <span
              ...
              <div
                className={`grid gap-3 transition-[max-height,padding] duration-500 ease-in-out overflow-hidden ${
                  openYear === year
                    ? "max-h-[1200px] p-4 grid-cols-[repeat(auto-fill,minmax(200px,1fr))]"
                    : "max-h-0 p-0"
                }`}
              >
                {groupedRanks[year].map((r, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-[auto_1fr] gap-3 items-center bg-white border border-slate-200 rounded-xl p-3 shadow-md hover:shadow-lg hover:border-amber-400 transition"
                  >
                    <span className="h-8 px-3 min-w-8 flex items-center justify-center rounded-full font-extrabold text-slate-950 text-xs sm:text-sm bg-gradient-to-r from-amber-400 to-amber-500 shadow-sm">
                      {r.rank}
                    </span>
                    <div>
                      ...
                      <p className="text-slate-600 text-xs font-semibold">{r.class}</p>
                    </div>
                  </div>
                ))}
              </div>
